#!/bin/bash

cd "$(dirname "$0")/.."

# 2. 인자 받아오기
ALGO=$1        # 예: ddqn, mappo, llm_mca
ENV=$2         # 예: Foraging-8x8-2p-2f-coop-v3
USE_WANDB=$3   # true or false
GROUP_NAME=$4  # wandb 그룹명 (미지정 시 알고리즘명)

# 3. 기본값 설정
if [ -z "$ALGO" ]; then ALGO="llm_mca"; fi
if [ -z "$ENV" ]; then ENV="Foraging-8x8-2p-2f-coop-v3"; fi
if [ -z "$USE_WANDB" ]; then USE_WANDB="false"; fi
if [ -z "$GROUP_NAME" ]; then GROUP_NAME="$ALGO"; fi

# 4. 하이퍼파라미터 설정 (train.py argparse에 맞춰 구성)
# 4-1. 공통
GRAD_STEPS=60
MAX_STEPS=25           # 15는 5x5에서도 보상 경험을 지나치게 잘라냄
BATCH_SIZE=64
GAMMA=0.99
EVAL_EPISODES=100      # LBF의 10-episode 평가는 분산이 너무 큼
LOG_EVERY=5

# 4-2. LLM 계열 예산
LLM_ITERATIONS=100
LLM_EPISODES_PER_ITER=4
MODEL=Qwen/Qwen2.5-7B-Instruct
MAX_NEW_TOKENS=1200
# Qwen은 긴 batch 출력에서 배열 누락/길이 오류가 잦다. Episode별 호출이 현재 가장
# 안정적이며, 남는 포맷 오류는 한 번만 재시도한다.
CRITIC_GROUP=1
CRITIC_RETRIES=1

# 4-3. 비-LLM 계열 예산 (LLM 호출이 없어 크게)
RL_ITERATIONS=1000
RL_EPISODES_PER_ITER=64    # PPO/DQN 배치

if [ "$ALGO" == "llm_mca" ] || [ "$ALGO" == "llm_taca" ]; then
    ITERATIONS=$LLM_ITERATIONS
    EPISODES_PER_ITER=$LLM_EPISODES_PER_ITER
else
    ITERATIONS=$RL_ITERATIONS
    EPISODES_PER_ITER=$RL_EPISODES_PER_ITER
fi

LOG_DIR=results/lbf
mkdir -p "$LOG_DIR"

EXTRA_ARGS=""
if [ "$ALGO" == "llm_mca" ] || [ "$ALGO" == "llm_taca" ]; then
    EXTRA_ARGS="--model $MODEL \
                --max_new_tokens $MAX_NEW_TOKENS \
                --critic_group $CRITIC_GROUP \
                --critic_retries $CRITIC_RETRIES"
fi
# wandb: project는 train.py가 AAMAS_{env}로 자동 설정, run 이름은 {algo}_seed{seed}
if [ "$USE_WANDB" == "true" ]; then
    EXTRA_ARGS="$EXTRA_ARGS --use_wandb --group $GROUP_NAME"
fi

# 5. 시드별 루프 실행
for i in 0 1 2
do
    echo "Starting Experiment: $ALGO on $ENV with SEED $i"
    python train.py \
        --algo $ALGO \
        --env $ENV \
        --seed $i \
        --iterations $ITERATIONS \
        --episodes_per_iter $EPISODES_PER_ITER \
        --grad_steps $GRAD_STEPS \
        --max_steps $MAX_STEPS \
        --batch_size $BATCH_SIZE \
        --gamma $GAMMA \
        --eval_episodes $EVAL_EPISODES \
        --log_every $LOG_EVERY \
        $EXTRA_ARGS \
        2>&1 | tee "$LOG_DIR/${ALGO}_${ENV}_seed${i}.log"
done
