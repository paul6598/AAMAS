...
class RSVPRunner:
    ...
    def __init__(self, args, logger):
        ...
        self._shaping_sums = []
        # Aggregated shaping diagnostics.  These are deliberately sums over
        # train transitions (rather than per-step F/r ratios, which explode
        # whenever the sparse environment reward is zero).
        ...
        if self.commander is not None:
            ...
            self._phase_log = open(os.path.join(pdir, fname), "a")
            self._last_phase_key = None
            # opt-in per-step scheduler trace for diagnostics (Q-003 P0);
            # pure logging - decision order and RNG untouched
            if getattr(args, "sched_trace", False):
                tdir = os.path.join(repo_root, "results", "trace")
                os.makedirs(tdir, exist_ok=True)
                self._trace = open(os.path.join(tdir, fname), "a")
    ...
    def _maybe_refresh(self, snap, x, test_mode):
        ...
        if ready:
            # Q-004 fix (approved 9/6): an episode counts as "armed" for the
            # h budget controller whenever the CUSUM path was live in it, not
            # only when a new armed issuance happened (episode-spanning spells
            # were dropping ~76-86% of armed episodes from the window)
            self._ep_armed = True
        if not due and ready:
            ...
            v_num = max(0.0, self._vF(x, self._ref_heads))
            v_den_raw = self._vF(self._x_ref, self._ref_heads)
        ...
        if self._trace is not None:
            self._trace.write(json.dumps({
                ...
                "since": since, "armed": bool(ready), "num": v_num,
                "den_raw": v_den_raw, "v": v, "S": round(self._S, 4),
                "h": round(self.h, 3), "due": due, "early": early}) + "\n")
        if not due:
            return
            ...
            if not test_mode and getattr(self, "_phase_log", None) is not None:
                ...
                if pkey != self._last_phase_key:
                    self._phase_log.write(json.dumps(
                        {"t_global": self.t_env + self.t, "key": pkey}) + "\n")
                    self._last_phase_key = pkey
...
# Legacy import path/class name; the implementation is RSVP-only.
