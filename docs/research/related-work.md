...
| **Canonaco, Restelli, Roveri ECAI 2020 (NSD-RL)** | 두 학습 반복(i, i−k)의 배치를 공통 평가 정책 π_μ로 **중요도 샘플링**해 기대 리턴을 비교하는 부트스트랩 가설검정(분포 가정 없음) → 검정 결과(+1/0)를 **CUSUM으로 누적** m_i = max(0, m_{i−1}+Te), m_i ≥ K에서 변화 선언. π_μ는 Rényi 발산 최소화로 선택(IS 분산 통제·type I 오류 보전). K는 ARL₀(α)=u(I−P_α)⁻¹1 (마르코프 체인 흡수시간)로 설정. 적응 = Adam 모멘트 리셋. Pendulum/MountainCar, 지연 ≈4–5 반복, FPR 0 | 우리와의 대응: "성능이 바뀌었는가"를 학습기 리턴으로 재는 **③층(성능 게이트)** 의 정석. 통계량 = 두 시점의 리턴 비교이므로 반응이 느리고(반복 단위), 변화 원인(환경 vs 정책)을 구분 못 함. ARL₀로 임계값을 정하는 절차는 우리 예산→h 보정에 그대로 차용 |
| Padakandla et al. 2020 (Context Q-learning + ODCP) | 경험 스트림의 온라인 변화점 검출 → 컨텍스트별 Q 테이블 | 모델 없이 경험 통계량에 거는 대안 |
| Li, Shi, Wu, Fryzlewicz **NeurIPS 2025** (arXiv 2203.01707, CUSUM-RL) | 오프라인 데이터 D={(S,A,R)}_{N×T}에서 H₀: Q*_t = Q* ∀t≥T₀ 검정. 후보 변화점 u마다 [T₀,u]·[u,T]에서 FQI(시브 기저)로 Q 추정 → 차이의 ℓ₁/max/정규화-max **CUSUM형 통계량**(경계 가중 τ_u), 다중자 부트스트랩으로 p값. 정리: type I 오류 점근 제어(Thm 4.1), 검정력(Thm 4.2, 신호 Δ ≫ (1−γ)⁻²√(L log(NT)/εNT)). 절차: 최근 창부터 **후방 탐색**으로 마지막 정상 구간 찾기 → 그 구간 데이터로만 정책 학습(온라인은 배치마다 반복). 실험: 시뮬레이션 + Intern Health Study. 한계: 이산 행동, 유일 최적정책·마진 조건, 확률적 전이 필요, 국소화 속도 미도출 | 검정 대상이 **Q(가치)** 라는 점에서 우리 "효과 공간" 관점과 부합. 오프라인·배치형이라 우리의 스텝 단위 갱신 트리거보다는 **리플레이 버퍼의 어느 구간이 현 가이던스 하에서 정상인가**(hindsight 재계산/버퍼 절단)에 맞는 도구. 부트스트랩 캘리브레이션과 경계 가중은 우리 재생 통계에 차용 가능 |
...
### 4c-2. "이론적 근거 하에 LLM 호출 시점을 정한" 연구 — 현황 (2026-08-31 확인)

| 연구 | 설정 | 이론 | 우리와의 관계 |
|---|---|---|---|
| **Wang (USC) 2026, arXiv 2607.13048** "Uncertainty-Aware Sequential Decision Rules for Event-Triggered LLM Invocation in Streaming Systems" | 센서 스트림(C-MAPSS 터보팬, CIC-IDS2017) 이상 진단. 빠른 모델이 상시, LLM은 트리거 시 "의미적 진단" 생성. **RL 에이전트 없음, 멀티에이전트 없음, 가이던스 낡음 개념 없음** | LLM 호출 = 위험 기반 순차 정지 문제. 이벤트 트리거·최적 정지·SPRT·**CUSUM**·베이지안을 특수 사례로 포괄. 정리: 임계값 정책 최적성, 최소 트리거 간격, 정상 O(√T log T)·변화점 C_T개 하 O(√((C_T+1)T log T)) regret, OGD/LinUCB 적응 임계값 수렴 | **가장 가까운 이론 선례.** 목적함수(호출 비용 최소 s.t. miss율 ≤ ε)가 우리 §3e와 동형. 핵심 실증 "주기·무작위 샘플링은 같은 예산에서 검출률 ≈ 0 — **호출 빈도가 아니라 타이밍**"은 우리 논지의 타 도메인 증거. 차별점: 우리는 (i) 호출 결과가 *정책을 바꾸는 가이던스*라 낡음 비용이 행동으로 전파, (ii) 팀 공유·오프폴리시 학습, (iii) 검출 대상이 외부 조언자 출력의 유효성 |
| Goel et al. (UT Austin/Honda) 2026, arXiv 2608.18490 BayesBeliefAgent | Overcooked/Burrito, LLM 에이전트 간 협업(학습 정책 아님) | 베이지안 역계획으로 파트너 모델; **우도 기반 모순 감지**(θ_obs) + 안정성/신뢰도 게이팅으로 재계획. 최적성 정리 없음 | 재계획 2.0/ep vs 주기-10 43/ep, 보상 동등 이상 → "타이밍 > 빈도" 또 하나의 증거. 감지 대상은 파트너 의도(②계열) |
| When2Ask (Hu 2023), Learning When to Plan (2025), LLM-MARL 게이트 | LLM-guided RL/에이전트 | RL 또는 지도학습으로 게이트 학습. 검출 이론 없음 | 학습 게이트 베이스라인 |
| Harb 2018 deliberation cost; Da Silva 2020 RCMP(불확실성 임계값+예산 조언); Torrey & Taylor 2013 | 옵션 종료 / 조언 예산 | 규제항·휴리스틱, 최적성 없음 | 프레이밍 뿌리 |

**결론.** LLM-guided **RL/MARL**에서 검출 이론(QCD/CUSUM)에 근거해 호출 시점을
정한 연구는 확인되지 않음. 스트리밍 진단 도메인에서 2026년 동형 프레임(Wang)이
나왔으므로, 우리는 이를 "이론 선례"로 인용하고 (i)–(iii)으로 차별화한다. 두 2026
논문 모두 "같은 예산에서 주기 갱신 ≪ 이벤트/모순 기반 갱신"을 보였다는 점은
S0/E2의 A5 대 A2 비교가 어떤 결과를 내야 하는지의 사전 기대치가 된다.
